...
import { Avatar, Card, CardContent, Grid, LinearProgress, Stack, Typography } from '@mui/material'
...
export default function OverviewPanel({ plants }: { plants: Plant[] }) {
  ...
  const trend = [
    { date: 'Oct 20', healthy: 2, unhealthy: 1, needsCheck: 0 },
    { date: 'Oct 21', healthy: 2, unhealthy: 1, needsCheck: 0 },
    { date: 'Oct 22', healthy: 3, unhealthy: 0, needsCheck: 2 },
    { date: 'Oct 23', healthy: 3, unhealthy: 0, needsCheck: 2 },
    { date: 'Oct 24', healthy: 2, unhealthy: 1, needsCheck: 1 },
    { date: 'Oct 25', healthy: 4, unhealthy: 1, needsCheck: 1 },
  ];
  ...
  return (
    <Grid container spacing={3}>
          ...
          <CardContent>
            <Stack spacing={2}>
              ...
              <Typography variant="body2" color="text.secondary">
                Last health check: {lastChecked ? new Date(lastChecked).toLocaleString() : 'N/A'}
              </Typography>
              <Stack spacing={1}>
      ...
      <Grid item xs={12} md={4}>
        <Card sx={{ height: '100%' }}>
          <CardContent sx={{ height: 240, display: 'flex', flexDirection: 'column' }}>
            <Typography variant="subtitle1" mb={1}>Health Distribution</Typography>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} dataKey="value" innerRadius="60%" outerRadius="80%" paddingAngle={5} labelLine={false}>
          ...
          <CardContent sx={{ height: 240, display: 'flex', flexDirection: 'column' }}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend}>
                ...
                <Legend iconType="circle" />
                <Line type="monotone" dataKey="healthy" stroke={COLORS[0]} strokeWidth={2} dot={false} name="Healthy" />
                <Line type="monotone" dataKey="unhealthy" stroke={COLORS[1]} strokeWidth={2} dot={false} name="Unhealthy" />
                <Line type="monotone" dataKey="needsCheck" stroke={COLORS[2]} strokeWidth={2} dot={false} name="Needs Check" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
